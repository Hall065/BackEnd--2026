    ...
    <div class="dash-root py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="fade-up">
                ...
                <h1 class="dash-greeting">Editar <span>item do estoque</span></h1>
            </div>

            <div class="section-card fade-up delay-1">
                <div class="gold-divider"></div>
                <h2 class="section-title mb-6">Informações do Estoque</h2>

                <form action="{{ route('estoque.update', $estoque->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="produto_id" class="dash-label">Produto</label>
                            <select id="produto_id" name="produto_id" class="dash-input" required autofocus>
                        ...
                        <div>
                            <label for="qntd" class="dash-label">Quantidade</label>
                            <input id="qntd" name="qntd" type="number" class="dash-input" value="{{ old('qntd', $estoque->qntd) }}" required min="0" />
                            @error('qntd')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-center gap-6 pt-4 border-t border-[#e8e0d0] mt-6">
                        ...
                        <a href="{{ route('estoque.index') }}" class="btn-cancel">
                            Cancelar
                        </a>
                    </div>
                </form>
