<x-app-layout>
    ...
    <style>
        ...
        .stars-option + label {
            ...
            border: 1px solid #e8e0d0; border-radius: 1px;
    ...
    <div class="dash-root py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="fade-up">
                ...
                <h1 class="dash-greeting">Adicionar <span>personagem</span></h1>
            </div>
                ...
                <form action="{{ route('characters.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input name="name" type="text" class="dash-input" value="{{ old('name') }}" required autofocus placeholder="Ex: Hu Tao" />
                            @error('name') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="dash-label">Constelação</label>
                            <input name="constellation" type="text" class="dash-input" value="{{ old('constellation') }}" placeholder="Ex: Papilio Charontis" />
                            @error('constellation') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                    </div>

                    {{-- Elemento, Arma e Região --}}
                        ...
                        <div>
                            ...
                            <select name="element" class="dash-input" required>
                                ...
                                @foreach(['Geo','Pyro','Hydro','Cryo','Electro','Dendro','Anemo'] as $el)
                                    <option value="{{ $el }}" {{ old('element') == $el ? 'selected' : '' }}>{{ $el }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="weapon" class="dash-input" required>
                                ...
                                @foreach(['Sword','Claymore','Polearm','Bow','Catalyst'] as $w)
                                    <option value="{{ $w }}" {{ old('weapon') == $w ? 'selected' : '' }}>{{ $w }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="stars-selector">
                            <input type="radio" name="stars" id="stars4" value="4" class="stars-option" {{ old('stars', '4') == '4' ? 'checked' : '' }}>
                            <label for="stars4">★★★★ — 4 estrelas</label>

                            <input type="radio" name="stars" id="stars5" value="5" class="stars-option" {{ old('stars') == '5' ? 'checked' : '' }}>
                            <label for="stars5">★★★★★ — 5 estrelas</label>
                        </div>
                        @error('stars') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    {{-- Playstyle e Afiliação --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="dash-label">Playstyle</label>
                            <input name="playstyle" type="text" class="dash-input" value="{{ old('playstyle') }}" placeholder="Ex: Main DPS / Burst carry" />
                            @error('playstyle') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="dash-label">Afiliação</label>
                            <input name="affiliation" type="text" class="dash-input" value="{{ old('affiliation') }}" placeholder="Ex: Liyue Qixing" />
                    ...
                    <div>
                        ...
                        <textarea name="mechanics" class="dash-input" rows="3" placeholder="Descreva as mecânicas do personagem...">{{ old('mechanics') }}</textarea>
                    ...
                    <div>
                        ...
                        <input name="image_url" type="url" class="dash-input" value="{{ old('image_url') }}" placeholder="https://genshin.jmp.blue/characters/hu-tao/portrait" />
                        <p class="hint">Deixe em branco para gerar automaticamente via genshin.jmp.blue pelo nome do personagem.</p>
                    ...
                    <div class="flex items-center gap-6 pt-4 border-t border-[#e8e0d0]">
                        <button type="submit" class="btn-submit">Salvar Personagem</button>
