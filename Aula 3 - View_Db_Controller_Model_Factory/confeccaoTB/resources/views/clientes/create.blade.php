    ...
    <div class="dash-root py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="fade-up">
                ...
                <h1 class="dash-greeting">Adicionar <span>cliente</span></h1>
            </div>
            ...
            <div class="section-card fade-up delay-1">
                ...
                <form action="{{ route('clients.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="nome" name="name" type="text" class="dash-input" required autofocus />
                        @error('name')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="cpf" name="cpf" type="text" class="dash-input" required />
                            @error('cpf')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input id="telefone" name="telefone" type="text" class="dash-input" required />
                            @error('telefone')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" class="dash-input" required />
                        @error('email')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="endereco" name="endereco" class="dash-input" rows="3"></textarea>
                        @error('endereco')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
