    ...
    <div class="dash-root py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="fade-up">
                ...
                <h1 class="dash-greeting">Adicionar <span>Fornecedor</span></h1>
            </div>

            <div class="section-card fade-up delay-1">
                <div class="gold-divider"></div>
                <h2 class="section-title mb-6">Dados do Fornecedor</h2>

                <form action="{{ route('fornecedores.store') }}" method="POST" class="space-y-6">
                    @csrf

                    <div>
                        <label for="nome" class="dash-label">Razão Social / Nome</label>
                        <input id="nome" name="nome" type="text" class="dash-input" value="{{ old('nome') }}" required autofocus />
                        @error('nome')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="cnpj" class="dash-label">CNPJ</label>
                            <input id="cnpj" name="cnpj" type="text" class="dash-input" value="{{ old('cnpj') }}" required />
                            @error('cnpj')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input id="telefone" name="telefone" type="text" class="dash-input" value="{{ old('telefone') }}" required />
                            @error('telefone')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        <label for="email" class="dash-label">E-mail corporativo</label>
                        <input id="email" name="email" type="email" class="dash-input" value="{{ old('email') }}" required />
                        @error('email')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-6 pt-4 border-t border-[#e8e0d0]">
                        ...
                        <a href="{{ route('fornecedores.index') }}" class="btn-cancel">
                            Cancelar
                        </a>
                    </div>
                </form>
