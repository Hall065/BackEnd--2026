    ...
    <div class="dash-root py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="fade-up">
                ...
                <h1 class="dash-greeting">Adicionar <span>Pedido</span></h1>
            </div>

            <div class="section-card fade-up delay-1">
                <div class="gold-divider"></div>
                <h2 class="section-title mb-6">Detalhes do Pedido</h2>
                ...
                <form action="{{ route('pedidos.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="client_id" class="dash-label">ID do Cliente</label>
                            <input id="client_id" name="client_id" type="number" class="dash-input" value="{{ old('client_id') }}" required autofocus />
                            @error('client_id')
                        ...
                        <div>
                            <label for="valor" class="dash-label">Valor (R$)</label>
                            <input id="valor" name="valor" type="number" step="0.01" min="0" class="dash-input" value="{{ old('valor') }}" required />
                            @error('valor')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="dash-input" required>
                            ...
                            <option value="em producao" {{ old('status') == 'em producao' ? 'selected' : '' }}>Em Produção</option>
                    ...
                    <div class="flex items-center gap-6 pt-4 border-t border-[#e8e0d0]">
                        ...
                        <a href="{{ route('pedidos.index') }}" class="btn-cancel">
                            Cancelar
                        </a>
                    </div>
                </form>
